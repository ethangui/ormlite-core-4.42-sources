...
/**
 * Base class for all of the {@link DatabaseType} classes that provide the
 * per-database type functionality to create tables and build queries.
 * 
 * <p>
 * Here's a good page which shows some of the <a
 * href="http://www.1keydata.com/sql/sql-primary-key.html" >differences between
 * SQL versions</a>.
 * </p>
 * 
 * @author graywatson
        ...
        if (className != null)
        {
            ...
            catch (ClassNotFoundException e)
            {
                throw SqlExceptionUtil
                        .create("Driver class was not found for "
                                + getDatabaseName()
                                + " database.  Missing jar with class "
                                + className
                                + ".", e);
            }
        }
        ...
        throws SQLException
    {
        ...
        default:
            ...
            throw new IllegalArgumentException("Unknown SQL-type "
                                               + dataPersister.getSqlType());
        }
        sb.append(' ');

        /*
         * NOTE: the configure id methods must be in this order since
         * isGeneratedIdSequence is also isGeneratedId and isId. isGeneratedId
         * is also isId.
         */
        if (fieldType.isGeneratedIdSequence() && !fieldType.isSelfGeneratedId())
        {
            configureGeneratedIdSequence(sb,
                                         fieldType,
                                         statementsBefore,
                                         additionalArgs,
                                         queriesAfter);
        ...
        {
            configureGeneratedId(tableName,
                                 sb,
                                 fieldType,
                                 statementsBefore,
                                 statementsAfter,
        ...
        // if we have a generated-id then neither the not-null nor the default
// make sense and cause syntax errors
     ...
     *            TODO
     */
    protected void appendStringType(
        StringBuilder sb,
        FieldType fieldType,
        int fieldWidth)
    {
     ...
     *            TODO
     */
    protected void appendLongStringType(
        StringBuilder sb,
        FieldType fieldType,
        int fieldWidth)
    {
     ...
     *            TODO
     */
    protected void appendBooleanType(
        StringBuilder sb,
        FieldType fieldType,
        int fieldWidth)
    {
     ...
     *            TODO
     */
    protected void appendShortType(
        StringBuilder sb,
        FieldType fieldType,
        int fieldWidth)
    {
     ...
     *            TODO
     */
    private void appendIntegerType(
        StringBuilder sb,
        FieldType fieldType,
        int fieldWidth)
    {
     ...
     *            TODO
     */
    protected void appendByteArrayType(
        StringBuilder sb,
        FieldType fieldType,
        int fieldWidth)
    {
     ...
     *            TODO
     */
    protected void appendSerializableType(
        StringBuilder sb,
        FieldType fieldType,
        int fieldWidth)
    {
     ...
     *            TODO
     */
    protected void appendBigDecimalNumericType(
        StringBuilder sb,
        FieldType fieldType,
        int fieldWidth)
    {
    ...
    /**
     * Output the SQL necessary to configure a generated-id column. This may add
     * to the before statements list or additional arguments later.
     * 
     * NOTE: Only one of configureGeneratedIdSequence, configureGeneratedId, or
     * configureId will be called.
     */
    protected void configureGeneratedIdSequence(
        ...
        throws SQLException
    {
        throw new SQLException("GeneratedIdSequence is not supported by database "
                               + getDatabaseName()
                               + " for field "
                               + fieldType);
    }
    ...
    protected void configureGeneratedId(
        ...
        List<String> queriesAfter)
    {
        throw new IllegalStateException("GeneratedId is not supported by database "
                                        + getDatabaseName()
                                        + " for field "
                                        + fieldType);
    }
    ...
    /**
     * Output the SQL necessary to configure an id column. This may add to the
     * before statements list or additional arguments later.
     * 
     * NOTE: Only one of configureGeneratedIdSequence, configureGeneratedId, or
     * configureId will be called.
     */
...
//            System.out.println("indexName = " + indexName);
            ...
            {
//                System.out
//                        .println("Inside. Caught " + fieldType.getIndexName());
                if (sb == null)
                    sb = new StringBuilder(48);
                if (!firstTime)
                    sb.append(",");
                sb.append("KEY ");
                appendEscapedEntityName(sb, fieldType.getTableName()
                                            + "_"
     ...
     * {@link #addPrimaryKeySql} method at the end. If false then it needs to be
     * done by hand inline.
     */
    ...
    public void appendSelectNextValFromSequence(
        StringBuilder sb,
        String sequenceName)
    {
        // noop by default.
     ...
     *             for sub classes.
     */
    public <T> DatabaseTableConfig<T> extractDatabaseTableConfig(
    ...
    /**
     * If the field can be nullable, do we need to add some sort of NULL SQL for
     * the create table. By default it is a noop. This is necessary because
     * MySQL has a auto default value for the TIMESTAMP type that required a
     * default value otherwise it would stick in the current date automagically.
     */
    ...
    /**
     * Conversion to/from the Boolean Java field as a number because some
     * databases like the true/false.
     */
        ...
        implements FieldConverter
    {
        ...
        public Object resultStringToJava(
            FieldType fieldType,
            String stringValue,
            int columnPos)
        {
